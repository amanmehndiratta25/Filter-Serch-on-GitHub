...
class Login
{
    private static  $session_values=""; private $msgg_P=NULL; private $CHK=NULL;
     public function __construct() { 
        ...
        $this->session_values=$_SESSION['userActive'];
        
        if($this->session_values=="userActive"){
            header("Location:".SITEURL."Cpanel/index.php");
        }else if($_GET['msgg']!=NULL) {
            $this->msgg_P=$_GET['msgg'];
            echo "<p style='background-color:orange; width:100%; height:50px;color:white; font-weight:900; font-size:25px;'>$this->msgg_P<span style='float:right;'><a href=".SITEURL.">View Site</a></span></p>";
        }
    }
    // Close Constructor
    public  function loginDetails($user,$pass){
        
        $user_obj= new User();
        $this->CHK=$user_obj->checkUser($user, $pass);
        
        if ($this->CHK['final_rs']==1){             
            $this->session_values=$_SESSION['userActive'] = $this->CHK['row'][0]['status'];
            $this->session_values=$_SESSION['userDetails'] = $this->CHK['row'][0];
            header("Location:".SITEURL."Cpanel/index.php");
        }else {
            echo "<p style='background-color:orange; width:100%; height:50px;color:white; font-weight:900; font-size:25px;'>$this->CHK<span style='float:right;'><a href=".SITEURL.">View Site</a></span></p>";
        }
    }
}
// Close Login Class
...
if(isset($_REQUEST['user'], $_REQUEST['pass'])){
    ...
    $login_obj->loginDetails($user,$pass);
}
?>
...
<form action="" method="post" name="loginForm">
  ...
  <div class="txt">
    <input id="pwd" type="password" name="pass" placeholder="Password" />
